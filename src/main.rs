...
fn shuffle(users: &mut Vec<String>) {
    let mut rng = rng();
...
fn generate_pairings(users: &Vec<String>) -> Vec<(String, String)> {
...
fn main() {
    ...
    println!("Before shuffle: {:?}", users);
    shuffle(&mut users);
    println!("After shuffle: {:?}", users);

    let pairings = generate_pairings(&users);
